...
namespace scnu{
    ...
    void edge_bipartite_core_maintenance::batch_insert(const shared_ptr<abstract_bipartite_graph> &B,
        ...
        *delta = find_core(B);
        ...
        for(uint32_t alpha = 1; alpha <= *delta;++alpha){
            pool->submit_task([=]{
                ...
                for (const auto &e: *edge_set) {
                    ...
                    auto min_value = std::min(left_index_map->at(l)->get_j(alpha), right_index_map->at(r)->get_maximal_j(alpha));
                    if (min_value < phi_alpha) {
                        phi_alpha = min_value;
                    }
                }
        ...
        for(uint32_t beta = 1; beta<= *delta; ++beta){
            pool->submit_task([=]{
                ...
                for(const auto &e:*edge_set){
                    ...
                    auto min_value = std::min(left_index_map->at(l)->get_maximal_i(beta), right_index_map->at(r)->get_i(beta));
                    if(min_value < phi_beta){
                        phi_beta = min_value;
                    }
                }
        ...
        for(uint32_t alpha = 1; alpha <= *delta;++alpha){
            pool->submit_task([=]{
                batch_update_alpha_phi_alpha_core(B,global_left_mutex, global_right_mutex, new_left_index_map, new_right_index_map, alpha, alpha_vector->at(alpha));
            });
        }

        for(uint32_t beta = 1; beta<= *delta; ++beta){
           pool->submit_task([=]{
               batch_update_phi_beta_beta_core(B, global_left_mutex, global_right_mutex,new_left_index_map, new_right_index_map, beta_vector->at(beta), beta);
           });
                                                       ...
                                                       const shared_ptr<thread_pool>& pool)
    {
        ...
        for(uint32_t alpha = 1;alpha <= *delta; ++alpha){
            ...
            pool->submit_task([=]{
                batch_update_alpha_tau_alpha_core(B,global_left_mutex, global_right_mutex,new_left_index_map, new_right_index_map, alpha,alpha_vector->at(alpha));
            });
        }
        ...
        for(uint32_t beta = 1; beta <= *delta; ++beta){
            ...
            pool->submit_task([=]{
                batch_update_tau_beta_beta_core(B,global_left_mutex, global_right_mutex,new_left_index_map,new_right_index_map, beta_vector->at(beta), beta);
            });
        }
        pool->barrier();
    ...
    void edge_bipartite_core_maintenance::insert(const shared_ptr<abstract_bipartite_graph> &B,
        ...
        *delta = find_core(B);
        ...
        for(auto alpha = 1;alpha <= *delta;++alpha){
            pool->submit_task([=]{
                update_alpha_phi_alpha_core(B, global_left_mutex, global_right_mutex, left_index_map, right_index_map,
                                                        new_left_index_map, new_right_index_map, e, alpha);
            });
        }
        ...
        for(auto beta = 1;beta <= *delta;++beta){
            pool->submit_task([=]{
                update_phi_beta_beta_core(B, global_left_mutex, global_right_mutex, left_index_map, right_index_map,
                                                      new_left_index_map,
                                                      new_right_index_map, e, beta);
            });
        }
                                                 ...
                                                 const shared_ptr<thread_pool>& pool)
    {
        ...
        for (auto alpha = 1; alpha <= *delta; ++alpha) {
            pool->submit_task([=]{
                update_alpha_tau_alpha_core(B, global_left_mutex, global_right_mutex, left_index_map, right_index_map, new_left_index_map, new_right_index_map, e,
                                            alpha);
            });
        }
        ...
        for (auto beta = 1; beta <= *delta; ++beta) {
            pool->submit_task([=]{
                update_beta_tau_beta_core(B,global_left_mutex, global_right_mutex,  left_index_map, right_index_map, new_left_index_map, new_right_index_map, e,
                                          beta);
            });
        }
        pool->barrier();
                                                                    ...
                                                                    uint32_t beta)
    {
        ...
        for(const auto&r1:*Cr)
        {
            if(!new_right_index_map->count(r1)){
                new_right_index_map->insert({r1,make_shared<right_vertex_index>()});
            }
        ...
        for(const auto&l1:*Cl){
            if(!new_left_index_map->count(l1)) {
                new_left_index_map->insert({l1,make_shared<left_vertex_index>()});
            }
