...
#include "io/weighted_bipartite_graph_io.h"

namespace scnu{
    shared_ptr<vector<shared_ptr<weighted_bipartite_edge>>>
    weighted_bipartite_graph_io::get_edge_vector(const string &path,
                                                 const string &file_name) {
        ifstream input_stream(path + file_name);
        string line;
        auto edge_vector = make_shared<vector<shared_ptr<weighted_bipartite_edge>>>();
        while (getline(input_stream, line).good()) {
            ...
            auto edge = make_shared<weighted_bipartite_edge>(left_vertex_id, right_vertex_id, weight);
     ...
     * @return
     */
    shared_ptr<weighted_bipartite_graph> weighted_bipartite_graph_io::load_graph(const shared_ptr<vector<shared_ptr<weighted_bipartite_edge>>> &edge_vector) {
        auto graph = make_shared<weighted_bipartite_graph>();
        ...
        pool->submit_task([=] {
            for (const auto &edge: *edge_vector) {
                ...
                if (!left_vertex_map->count(l)) {
                    left_vertex_map->insert({l, make_shared<weighted_left_vertex>(l)});
        ...
        pool->submit_task([=] {
            for (const auto &edge: *edge_vector) {
                ...
                if (!right_vertex_map->count(r)) {
                    right_vertex_map->insert({r, make_shared<weighted_right_vertex>(r)});
    ...
    * @param output_path
    */
    void weighted_bipartite_graph_io::store_graph(const string &input_path, const string &output_path, uint32_t thread_number) {
        ...
        for (const auto &file_iter:std::filesystem::directory_iterator(input_path)) {
            ...
            pool.submit_task([=] {
                ...
                auto edge_set = make_shared<unordered_set<shared_ptr<weighted_bipartite_edge>, hash_weighted_bipartite_edge, equal_weighted_bipartite_edge>>();
                ...
                while (getline(input_stream, line).good()) {
                    ...
                    auto e = make_shared<weighted_bipartite_edge>(l, r,w);
                    edge_set->insert(e);
                }
                input_stream.close();
                ...
                for (const auto&e:*edge_set) {
                    output_stream << e->get_left_vertex_id()  << ',' <<(l_map->size() + e->get_right_vertex_id()) <<',' <<e->get_weight() << '\n';
                }
                output_stream.close();
            });
        }
