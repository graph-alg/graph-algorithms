...
#include "io/temporal_bipartite_graph_io.h"

namespace scnu{
    shared_ptr<vector<shared_ptr<temporal_bipartite_edge>>>
    temporal_bipartite_graph_io::get_edge_vector(const string &path,
                                                 const string &file_name) {
        ifstream input_stream(path + file_name);
        string line;
        auto edge_vector = make_shared<vector<shared_ptr<temporal_bipartite_edge>>>();
        while (getline(input_stream, line).good()) {
            ...
            double weight =  stod(line_vector->at(2));
            uint32_t timestamp = stoul(line_vector->at(3));
            auto edge = make_shared<temporal_bipartite_edge>(left_vertex_id, right_vertex_id, weight);
     ...
     * @return
     */
    shared_ptr<temporal_bipartite_graph> temporal_bipartite_graph_io::load_graph(const shared_ptr<vector<shared_ptr<temporal_bipartite_edge>>> &edge_vector) {
        auto graph = make_shared<temporal_bipartite_graph>();
        ...
        pool->submit_task([=] {
            for (const auto &edge: *edge_vector) {
                ...
                if (!left_vertex_map->count(l)) {
                    left_vertex_map->insert({l, make_shared<temporal_left_vertex>(l)});
        ...
        pool->submit_task([=] {
            for (const auto &edge: *edge_vector) {
                ...
                if (!right_vertex_map->count(r)) {
                    right_vertex_map->insert({r, make_shared<temporal_right_vertex>(r)});
    ...
    * @param output_path
    */
    void temporal_bipartite_graph_io::store_graph(const string &input_path, const string &output_path, uint32_t thread_number) {
        ...
        for (const auto &file_iter:std::filesystem::directory_iterator(input_path)) {
            ...
            pool.submit_task([=] {
                ...
                auto edge_map = make_shared<map<uint32_t, shared_ptr<unordered_set<shared_ptr<temporal_bipartite_edge>,
                        hash_temporal_bipartite_edge, equal_temporal_bipartite_edge>>>>();
                ...
                while (getline(input_stream, line).good()) {
                    ...
                    if(!edge_map->count(t)){
                        edge_map->insert({t, make_shared<unordered_set<shared_ptr<temporal_bipartite_edge>, hash_temporal_bipartite_edge,equal_temporal_bipartite_edge>>()});
                    }
                    auto e = make_shared<temporal_bipartite_edge>(l,r,w,t);
    ...
    void temporal_bipartite_graph_io::store_unique_graph(const string &input_path, const string &output_path, uint32_t thread_number) {
        auto directory = path(input_path);
        ...
        for (const auto &file_iter:std::filesystem::directory_iterator(input_path)) {
            ...
            pool.submit_task([=] {
                ...
                auto edge_map = make_shared<map<uint32_t, shared_ptr<unordered_set<shared_ptr<abstract_bipartite_edge>,
                        hash_abstract_bipartite_edge, equal_abstract_bipartite_edge>>>>();
                ...
                while (getline(input_stream, line).good()) {
                    ...
                    auto l = stoul(line_list->at(0));
                    auto r = stoul(line_list->at(1));
                    auto t = std::stoul(line_list->at(3));
                    ...
                    if(!edge_map->count(t)){
                        edge_map->insert({t, make_shared<unordered_set<shared_ptr<abstract_bipartite_edge>, hash_abstract_bipartite_edge,equal_abstract_bipartite_edge>>()});
                    }
                    auto e = make_shared<temporal_bipartite_edge>(l,r);
                    edge_map->at(t)->insert(e);
                }
                input_stream.close();
