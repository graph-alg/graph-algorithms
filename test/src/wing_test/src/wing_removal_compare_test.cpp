...
void wing_removal_compare_func(const shared_ptr<abstract_bipartite_graph> &G,
                               const shared_ptr<vector<shared_ptr<abstract_bipartite_edge>>> &total_removal_edge_vector,
                               const shared_ptr<unordered_map<shared_ptr<abstract_bipartite_edge>, uint32_t>> &previous_edge_wing_map,
                               const shared_ptr<unordered_map<shared_ptr<abstract_bipartite_edge>, uint32_t>> &previous_WS,
                               const shared_ptr<unordered_map<uint32_t, shared_ptr<extend_list<double, shared_ptr<abstract_bipartite_edge>>>>> &previous_wing_order_map,
...

    {
        ...
        auto order_time = Order_maintenance(G,removal_edge_vector,order_edge_wing_map,previous_WS,previous_wing_order_map,previous_rem,previous_k_max,thread_number);
...

    {
        ...
        auto bs_time = BS_maintenance(G,removal_edge_vector,quasi_edge_wing_map,previous_WS,previous_k_max,thread_number);

        if (wing_compare::same_associative_map(quasi_edge_wing_map, contrastive_edge_wing_map)) {
            LOG(logger, LOG_RANK::INFO) << "Basic Removal," << bs_time << "\n";
        }
    }
...
int main(int argc, char **argv) {
    ...
    string log_file_name = log_path + input_file_name + "_removal_compare_test.log";
    ...
    vector<uint32_t> m_array{10000};

    for (const auto &m: m_array) {
        wing_removal_compare_func(G,total_removal_edge_vector,previous_edge_wing_map,previous_WS,previous_wing_order_map,previous_rem,previous_k_max,logger,m,thread_number);
        std::cout<<std::endl;
    }
