...
void prepare(const shared_ptr<abstract_bipartite_graph>&G,
             ...
             const shared_ptr<unordered_map<uint32_t, shared_ptr<extend_list<double, shared_ptr<abstract_bipartite_edge>>>>> &previous_wing_order_map,
             const shared_ptr<unordered_map<shared_ptr<abstract_bipartite_edge>, uint32_t>> &previous_rem,
...
double decompose(const shared_ptr<abstract_bipartite_graph> &G,
    ...
    index_wing_decomposition::init(G, edge_mutex_map, edge_rank_map, edge_support_map, contrastive_edge_wing_map, pool);
    index_wing_decomposition::decompose(G, edge_mutex_map, edge_rank_map, edge_support_map, contrastive_edge_wing_map,pool);
...
double Order_maintenance(const shared_ptr<abstract_bipartite_graph> &G,
                         const shared_ptr<vector<shared_ptr<abstract_bipartite_edge>>> &insertion_edge_vector,
                         const shared_ptr<unordered_map<shared_ptr<abstract_bipartite_edge>, uint32_t>> &order_edge_wing_map,
                         const shared_ptr<unordered_map<shared_ptr<abstract_bipartite_edge>, uint32_t>> &previous_WS,
                         const shared_ptr<unordered_map<uint32_t, shared_ptr<extend_list<double, shared_ptr<abstract_bipartite_edge>>>>> &previous_wing_order_map,
                         const shared_ptr<unordered_map<shared_ptr<abstract_bipartite_edge>, uint32_t>> &previous_rem,
                         uint32_t previous_k_max,
...
double BS_maintenance(const shared_ptr<abstract_bipartite_graph> &G,
    ...
    quasi_wing_maintenance::insert(G, edge_mutex_map, insertion_edge_set, edge_rank_map, edge_support_map, quasi_edge_wing_map,
                                   ...
                                   k_max,
                                   pool);
    auto bs_time = bs_timer.get_elapse_second();
...
void wing_insertion_compare_func(const shared_ptr<abstract_bipartite_graph> &G,
                                 const shared_ptr<vector<shared_ptr<abstract_bipartite_edge>>> &total_insertion_edge_vector,
                                 const shared_ptr<unordered_map<shared_ptr<abstract_bipartite_edge>, uint32_t>> &previous_edge_wing_map,
                                 const shared_ptr<unordered_map<shared_ptr<abstract_bipartite_edge>, uint32_t>> &previous_WS,
                                 const shared_ptr<unordered_map<uint32_t, shared_ptr<extend_list<double, shared_ptr<abstract_bipartite_edge>>>>> &previous_wing_order_map,
    ...
    {
        double bottom_up_time = decompose(G,insertion_edge_vector,contrastive_edge_wing_map,thread_number);
...

    {
        ...
        auto order_time = Order_maintenance(G,insertion_edge_vector,order_edge_wing_map,previous_WS,previous_wing_order_map,previous_rem,previous_k_max,thread_number);
...
int main(int argc, char **argv) {
    ...
    string log_file_name = log_path + input_file_name + "_insertion_compare_test.log";
    ...
    auto G = load_graph(path, input_file_name, thread_number, total_insertion_edge_vector);


    /**
     * @brief prepare the information of the previous graph
     */
    ...
    {
        prepare(G,previous_edge_wing_map,previous_WS,previous_wing_order_map,previous_rem,previous_k_max,thread_number);
        LOG(logger, LOG_RANK::INFO) << "pre_k_max: " << previous_k_max << '\n';
    }
     ...
     */
    double edge_rate[] = {10000};
    for (auto m :edge_rate) {
        wing_insertion_compare_func(G,total_insertion_edge_vector,previous_edge_wing_map,previous_WS,previous_wing_order_map,previous_rem,previous_k_max,logger,m,thread_number);
        std::cout<<std::endl;
    }
    return 0;
}
