...
int main(int argc, char **argv) {
    ...
    std::string log_file_name = log_path + input_file_name + "_insertion_removal_test.log";
     ...
     */
    {
        ...
        if (wing_compare::same_associative_map(order_edge_wing_map, contrastive_edge_wing_map)) {
            LOG(logger, LOG_RANK::INFO) << "Order Maintenance," << order_time << "\n";
        }
...

    {
        ...
        quasi_wing_maintenance::insert(G, edge_mutex_map, insertion_edge_set, edge_rank_map, edge_support_map, quasi_edge_wing_map,
                                       ...
                                       previous_k_max3,
                                       pool);
        quasi_wing_maintenance::remove(G, edge_mutex_map, removal_edge_set, edge_rank_map,  quasi_edge_wing_map,WS,
                                       previous_k_max3,
                                       pool);
        ...
        if (wing_compare::same_associative_map(quasi_edge_wing_map, contrastive_edge_wing_map)) {
            LOG(logger, LOG_RANK::INFO) << "Basic Maintenance," << bs_time << "\n";
        }
...

    {
        ...
        quasi_wing_maintenance::insert2(G, edge_mutex_map, insertion_edge_set, edge_rank_map, edge_support_map, ws_edge_wing_map,
                                        ...
                                        previous_k_max3,
                                        pool);
        quasi_wing_maintenance::remove2(G, edge_mutex_map, removal_edge_set, edge_rank_map, ws_edge_wing_map,
                                        WS,
                                        previous_k_max3,
                                        pool);
        ...
        if (wing_compare::same_associative_map(ws_edge_wing_map, contrastive_edge_wing_map)) {
            LOG(logger, LOG_RANK::INFO) << "WS Maintenance," << ws_time << "\n";
        }
    ...
    /*{
        ...
        quasi_wing_maintenance::insert3(G, edge_mutex_map, insertion_edge_set, edge_rank_map, edge_support_map, bloom_edge_wing_map,
                                        ...
                                        k_max,
                                        pool);
        quasi_wing_maintenance::remove3(G, edge_mutex_map, removal_edge_set, edge_rank_map, bloom_edge_wing_map,
                                        WS,
                                        k_max,
                                        pool);
        auto bloom_time = bloom_timer.get_elapse_second();

        if (wing_compare::same_associative_map(bloom_edge_wing_map, contrastive_edge_wing_map)) {
            LOG(logger, LOG_RANK::INFO) << "Bloom Maintenance," << bloom_time << "\n";
        }
...

    {
        ...
        quasi_wing_maintenance::insert4(G, edge_mutex_map, insertion_edge_set, edge_rank_map, edge_support_map, wl_edge_wing_map,
                                        ...
                                        previous_k_max3,
                                        pool);
        quasi_wing_maintenance::remove4(G, edge_mutex_map, removal_edge_set, edge_rank_map, wl_edge_wing_map,
                                        WS,WL,
                                        previous_k_max3,
                                        pool);
        ...
        if (wing_compare::same_associative_map(wl_edge_wing_map, contrastive_edge_wing_map)) {
            LOG(logger, LOG_RANK::INFO) << "WL Maintenance," << wl_time << "\n";
        }
