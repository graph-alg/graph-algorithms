...
tuple<double, double, uint32_t> pms_maintenance(const shared_ptr<abstract_graph> &G,
    ...
    simple_timer maintenance_timer;
    auto pool = make_shared<thread_pool>(thread_number);
    auto loop_count = mixed_structure_truss_maintenance::remove(G, removed_edge_set, pms_edge_truss_map, pool);
    auto maintenance_time = maintenance_timer.get_elapse_second();
    auto memory_size = process_information::get_memory(getpid());

    G->insert_edge_collection(removal_edge_vector);

    return make_tuple(maintenance_time, memory_size, loop_count);
}
...
tuple<double, double, uint32_t> jes_order_maintenance(const shared_ptr<abstract_graph> &G,
    ...
    simple_timer maintenance_timer;
    auto pool = make_shared<thread_pool>(thread_number);
    auto loop_count = jes_order_truss_maintenance::remove(G, removed_edge_set, jes_edge_truss_map, truss_order_map,
                                                          rem, TS,
                                                          pool);
    auto maintenance_time = maintenance_timer.get_elapse_second();
...
//        }
    ...
    {
        auto [decomposition_time, memory_size,loop_count] = decompose(G, removal_edge_vector, contrast_edge_truss_map, thread_number);
...

    {
        ...
        auto [maintenance_time, memory_size,loop_count] = order_maintenance(G, removal_edge_vector, order_edge_truss_map, previous_edge_truss_support_map,
...

    {
        ...
        auto [maintenance_time, memory_size, loop_count] = pms_maintenance(G, removal_edge_vector, pms_edge_truss_map,
                                                                            thread_number);

        if (truss_compare::same_associative_map(contrast_edge_truss_map, pms_edge_truss_map)) {
            LOG(logger, LOG_RANK::INFO) << "PMS Removal," << maintenance_time << ","
                                        << double(memory_size) / 1024 / 1024 << "," << loop_count << "\n";
        }
    }

    {
        auto jes_edge_truss_map = container_copy::to_unordered_map<shared_ptr<abstract_edge>, uint32_t>(
                previous_edge_truss_map);
        auto [maintenance_time, memory_size, loop_count] = jes_order_maintenance(G, removal_edge_vector, jes_edge_truss_map,
                                                                                 previous_edge_truss_support_map, previous_truss_order_map,
                                                                                 previous_rem, thread_number);

        if (truss_compare::same_associative_map(contrast_edge_truss_map, jes_edge_truss_map)) {
