...
shared_ptr<abstract_graph> load_graph(const string &path,
    ...
    for (uint32_t i = size - m; i < size; ++i) {
    ...
    auto G = abstract_graph_io::load_graph(edge_vector, size - m, pool);
...
int main(int argc, char **argv) {
    ...
    string const log_file_name = log_path + "insertion_removal_size_test.log";
...
//        }
    ...
    for(const auto &rate:ratio_vector){
        ...
        auto G = load_graph(path, input_file_name, thread_number, rate, m, insertion_edge_vector, removal_edge_vector);
...
//        {
//            auto decomposition_time = decompose(G, insertion_edge_vector, removal_edge_vector, contrast_edge_truss_map, thread_number);
//
...
//        }
        ...
        {
            auto maintenance_time = order_maintenance(G, insertion_edge_vector, removal_edge_vector, order_edge_truss_map,
