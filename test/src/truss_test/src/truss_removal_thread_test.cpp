...
double order_maintenance(const shared_ptr<abstract_graph> &G,
    ...
    for (const auto &[k, k_list]: *previous_truss_order_map) {
        ...
        for (auto p = k_list->get_head(); p; p = p->get_next()) {
            truss_order_map->at(k)->push_back(p->get_value());
        }
    }
...
//    }
    ...
    double order_maintenance_time = order_maintenance(G, removal_edge_vector, order_edge_truss_map, previous_edge_truss_support_map,
    ...
    for(const auto &t:t_array){
        ...
        LOG(logger, LOG_RANK::INFO) << "Order Removal," << order_maintenance_time << "\n";

        {
            auto jes_edge_truss_map = container_copy::to_unordered_map<shared_ptr<abstract_edge>, uint32_t>(
                    previous_edge_truss_map);
            auto maintenance_time = jes_order_maintenance(G, removal_edge_vector, jes_edge_truss_map, previous_edge_truss_support_map,
