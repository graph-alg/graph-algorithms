...
tuple<double, double, uint32_t> pms_maintenance(const shared_ptr<abstract_graph> &G,
    ...
    simple_timer maintenance_timer;
    auto pool = make_shared<thread_pool>(thread_number);
    auto loop_count = mixed_structure_truss_maintenance::insert(G, inserted_edge_set, pms_edge_truss_map, pool);
    auto maintenance_time = maintenance_timer.get_elapse_second();
    auto memory_size = process_information::get_memory(getpid());
...
tuple<double, double, uint32_t> jes_order_maintenance(const shared_ptr<abstract_graph> &G,
    ...
    simple_timer maintenance_timer;
    auto pool = make_shared<thread_pool>(thread_number);
    auto loop_count = jes_order_truss_maintenance::insert(G, inserted_edge_set, parallel_edge_truss_map,
...
//        }
    ...
    auto contrast_edge_truss_map = make_shared<unordered_map<shared_ptr<abstract_edge>, uint32_t>>();
    {
        ...
        auto [decomposition_time, memory_size, loop_count] = decompose(G, insertion_edge_vector, contrast_edge_truss_map, thread_number);
...

    {
        ...
        auto [maintenance_time, memory_size, loop_count] = order_maintenance(G, insertion_edge_vector, order_edge_truss_map, previous_edge_truss_support_map,
...

    {
        ...
        auto [maintenance_time, memory_size, loop_count] = pms_maintenance(G, insertion_edge_vector, pms_edge_truss_map,
                                                                           thread_number);

        if (truss_compare::same_associative_map(contrast_edge_truss_map, pms_edge_truss_map)) {
            LOG(logger, LOG_RANK::INFO) << "PMS Insertion," << maintenance_time << ","
                                        << double(memory_size) / 1024 / 1024 << "," << loop_count << "\n";
        }
    }

    {
        auto parallel_edge_truss_map = container_copy::to_unordered_map<shared_ptr<abstract_edge>, uint32_t>(
                previous_edge_truss_map);
        auto [maintenance_time, memory_size, loop_count] = jes_order_maintenance(G, insertion_edge_vector, parallel_edge_truss_map,
