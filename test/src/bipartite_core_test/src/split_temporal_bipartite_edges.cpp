...
int main(int argc, char **argv) {
    ...
    for (auto &file_iter: std::filesystem::directory_iterator(input_path)) {
        ...
        pool->submit_task([=] {
            ...
            copy(half_begin, edge_vector->end(), inserter(*sub_edge_vector1, sub_edge_vector1->end()));

            abstract_bipartite_graph_io::output_csv_file(sub_edge_vector1, input_path + input_file_name+"-half2");
        });
    }
    pool->barrier();
}
