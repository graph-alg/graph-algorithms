...
//            shuffle(edge_vector->begin(),edge_vector->end(),*random_generator::get_default_engine(rd));
    ...
    */
    {
        auto edge_left_vertex_map = make_shared<unordered_map<uint32_t, shared_ptr<left_vertex_index>>>();
        auto edge_right_vertex_map = make_shared<unordered_map<uint32_t, shared_ptr<right_vertex_index>>>();

        for (const auto &[l, l_node]: *previous_left_index_map) {
            edge_left_vertex_map->insert({l, make_shared<left_vertex_index>(l_node)});
        }
        for (const auto &[r, r_node]: *previous_right_index_map) {
            edge_right_vertex_map->insert({r, make_shared<right_vertex_index>(r_node)});
        }
     ...
     */
    {
        ...
        simple_timer maintenance_timer;
        {
            ...
            edge_bipartite_core_maintenance::init(B, new_left_index_map, new_right_index_map, pool);
            edge_bipartite_core_maintenance::batch_insert(B, inserted_edge_set, edge_left_vertex_map, edge_right_vertex_map,
     ...
     */
    {
        ...
        simple_timer maintenance_timer;
        {
            ...
            branch_bipartite_core_maintenance::insert(B, left_core_degree_map, right_core_degree_map,
                                                      inserted_edge_set, branch_left_vertex_map, branch_right_vertex_map,
                                                      new_left_index_map, new_right_index_map,
                                                      pool);
        }
        auto maintenance_time = maintenance_timer.get_elapse_second();
