...
void query_compare_test(const shared_ptr<multiple_core>& mc,
                        const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_number_set_index>>> &vertex_core_number_set_map,
                        const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_pair_set_index>>> &vertex_core_pair_set_map,
                        const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_pair_map_index>>> &vertex_core_pair_map_map,
                        const string& log_path,
                        const string& input_file_name){
    const string log_file_name = log_path + "query_compare_test.log";
    scnu::simple_logger logger(log_file_name);
    ...
    const vector<uint32_t> number_list{20, 40, 60, 80, 100};
    for (const auto &num: number_list) {
        LOG(logger, LOG_RANK::INFO) << input_file_name << "," << num << "\n";
...
void query_density_test(const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_number_set_index>>> &vertex_core_number_set_map,
                        const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_pair_set_index>>> &vertex_core_pair_set_map,
                        const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_pair_map_index>>> &vertex_core_pair_map_map,
                        const string& log_path,
                        const string& input_file_name){
    const string log_file_name = log_path + "query_density_test.log";
    scnu::simple_logger logger(log_file_name);

    const vector<uint32_t> delta_list{4, 8, 12, 16, 20};
    for(const auto &delta:delta_list) {
        LOG(logger, LOG_RANK::INFO) << input_file_name << "," << delta << "\n";

        auto core_pair_set = make_shared<unordered_set<pair<uint32_t, uint32_t>, hash_pair>>();
        for (uint32_t i = 0; i < 30; i++) {
            core_pair_set->insert({delta + i, delta});
            core_pair_set->insert({delta, delta + i});
        }
...
void query_times_test(const shared_ptr<multiple_core>& mc,
                      const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_number_set_index>>> &vertex_core_number_set_map,
                      const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_pair_set_index>>> &vertex_core_pair_set_map,
                      const shared_ptr<unordered_map<uint32_t, shared_ptr<multiple_core_pair_map_index>>> &vertex_core_pair_map_map,
                      const string& log_path,
                      const string& input_file_name){
    const string log_file_name = log_path + "query_times_test.log";
    scnu::simple_logger logger(log_file_name);
    ...
    for(uint32_t i = 0; i < 5; ++i){
        LOG(logger,LOG_RANK::INFO) << input_file_name <<","<<i + 1<<'\n';
        std::shuffle(core_pair_vector->begin(),core_pair_vector->end(),*random_generator::get_default_engine(rd));

        auto core_pair_map = make_shared<unordered_set<pair<uint32_t, uint32_t>, hash_pair>>();

        for(uint32_t j = 0; j < 60;j++) {
            core_pair_map->insert(core_pair_vector->at(j));
        }

        auto base_core_pair_map =make_shared<unordered_map<pair<uint32_t, uint32_t>, shared_ptr<unordered_set<uint32_t>>, hash_pair>>();
        {
            simple_timer query_timer;
            multiple_core_query::query_multiple_core(vertex_core_number_set_map, core_pair_map, base_core_pair_map);
            auto query_time = query_timer.get_elapse_second();
...

        {
            ...
            simple_timer query_timer;
            multiple_core_query::query_multiple_core(vertex_core_pair_set_map, core_pair_map, sub_core_pair_map);
            auto query_time = query_timer.get_elapse_second();
...

        {
            ...
            simple_timer query_timer;
            multiple_core_query::query_multiple_core(vertex_core_pair_map_map, core_pair_map, sub_core_pair_map);
            auto query_time = query_timer.get_elapse_second();
...
int main(int argc, char ** argv){
    if(argc < 4){
        cout<<"Usage: path, file_name, depth, and thread_number!\n";
        exit(0);
    }
    ...
    */
    ...
    auto mc = make_shared<multiple_core>(base_map);
...
//
    ...
    query_compare_test(mc, vertex_core_number_set_map, vertex_core_pair_set_map, vertex_core_pair_map_map, log_path, input_file_name);

    query_density_test(vertex_core_number_set_map, vertex_core_pair_set_map, vertex_core_pair_map_map, log_path, input_file_name);

//    query_times_test(mc, vertex_core_number_set_map, vertex_core_pair_set_map, vertex_core_pair_map_map, log_path, input_file_name);
