...
void insertion_thread_test(const shared_ptr<temporal_graph>& G,
                           const shared_ptr<vector<shared_ptr<temporal_edge>>>& edge_vector,
                           uint32_t total_thread_number,
                           const string& log_path,
                           const string &input_file_name){
    const string log_file_name = log_path + "insertion_thread_test.log";
    ...
    auto previous_vertex_index_map = make_shared<unordered_map<uint32_t,shared_ptr<multiple_core_pair_map_index>>>();
    {
        ...
        auto pool = make_shared<thread_pool>(total_thread_number);
    ...
    const vector<uint32_t> ratio_list{1, 2, 4, 6, 10, 14};
    for (const auto &thread_number: ratio_list) {

        LOG(logger, LOG_RANK::INFO) << input_file_name << "," << thread_number << "\n";
...
void removal_thread_test(const shared_ptr<temporal_graph>& G,
                         const shared_ptr<vector<shared_ptr<temporal_edge>>>& edge_vector,
                         uint32_t total_thread_number,
                         const string& log_path,
                         const string &input_file_name){
    const string log_file_name = log_path + "removal_thread_test.log";
    scnu::simple_logger logger(log_file_name);
    ...
    auto previous_vertex_index_map = make_shared<unordered_map<uint32_t, shared_ptr<multiple_core_pair_map_index>>>();
    {
        ...
        branch_multiple_core_decomposition::init(G, vertex_mutex_map, vertex_edge_index_map, previous_vertex_index_map, pool);
        branch_multiple_core_decomposition::decompose(G, vertex_mutex_map, vertex_edge_index_map, previous_vertex_index_map, pool);
    }
...
void insertion_removal_thread_test(const shared_ptr<temporal_graph>& G,
                         const shared_ptr<vector<shared_ptr<temporal_edge>>>& edge_vector,
                         uint32_t total_thread_number,
                         const string& log_path,
                         const string &input_file_name){
    const string log_file_name = log_path + "insertion_removal_thread_test.log";
    scnu::simple_logger logger(log_file_name);
    ...
    for(const auto &thread_number:ratio_list) {
        ...
        auto base_map = make_shared<unordered_map<uint32_t, shared_ptr<multiple_core_pair_map_index>>>();
        {
            ...
            if (multiple_core_compare::same(base_map, base_map)) {
                LOG(logger, LOG_RANK::INFO) << "Branch Decomposition2*," << decomposition_time << "\n";
            }
...
//        }
    ...
    {
        insertion_thread_test(G, edge_vector, thread_number,log_path, input_file_name);
    }

    {
        removal_thread_test(G, edge_vector, thread_number,log_path, input_file_name);
    }

    {
        insertion_removal_thread_test(G, edge_vector, thread_number,log_path, input_file_name);
    }
